                ...
                if l.startswith("#") or 'CN' not in fields[8]:
                    ...
                    for sampleIndex in range(9,len(fields)):
                        ...
                        if two2one:
                            ...
                            sampleInfo[cnIndex] = str(max(list(map(int, cn))))
...
def check_java():
    ...
    jv = [x for x in str(subprocess.check_output("java -version", stderr=subprocess.STDOUT, shell=True), 'utf-8').split("\n") if x.find("version") > 0][0].split()[2].replace("\"", "")
...
def varsim_main(reference,
    ...
    variant_vcfs = list(map(os.path.realpath, variant_vcfs))
    ...
    if sv_insert_seq:
        ...
        variant_vcfs = list(map(os.path.realpath, in_vcfs))
    else:
    ...
    if not disable_vcf2diploid:
        ...
        contig_fastas = [os.path.join(out_dir, "%s_%s_%s.fa" % (x_y[0], sample_id, x_y[1])) for x_y in itertools.product(contigs, ["maternal", "paternal"])]
        fastas_to_cat = list(filter(os.path.isfile, contig_fastas))
        ...
        vcfs_to_cat = list(filter(os.path.isfile, [os.path.join(out_dir, "%s_%s.vcf" % (x, sample_id)) for x in contigs]))
    ...
    if simulator:
        ...
        fifo_src_dst = []
        if simulator == "dwgsim":
            for i, end in itertools.product(list(range(nlanes)), [1, 2]):
                fifo_src_dst.append(
                    ("simulated.lane%d.read%d.fastq" % (i, end),
                     "simulated.lane%d.read%d.fq.gz" % (i, end)))
        elif simulator == "art":
            for i, end, suffix in itertools.product(list(range(nlanes)), [1, 2], ["fq", "aln"]):
